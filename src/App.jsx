...
export default class App extends React.Component {
  ...
  componentDidUpdate(pp, ps) {
    if (ps.stage !== "camera" && this.state.stage === "camera")
      this.startCamera();
    if (ps.facing !== this.state.facing && this.state.stage === "camera")
      this.startCamera();
    if (
  ...
  async startCamera() {
    try {
      ...
      this.setState({
        cameraError: e && e.name ? e.name : "error",
        streamReady: false,
      });
    }
  }
  ...
  render() {
    ...
    const layoutAct = {
      borderColor: ACC,
      background: "rgba(168,105,76,0.10)",
    ...
    const soundStyle = merge(
      secondaryStyle,
      s.soundOn ? {} : { color: MUTED, background: "#F1ECE1" },
    );
    const primaryBtn = {
      display: "inline-flex",
      alignItems: "center",
    ...
    const cabinetStyle = curtains
      ? {
          ...
          boxShadow:
            "0 34px 64px -22px rgba(40,25,15,0.55), inset 0 1px 0 rgba(255,255,255,0.08)",
        }
      : {
          background: "#FBF7EF",
          padding: 0,
          borderRadius: "22px",
          boxShadow:
            "0 22px 52px -26px rgba(60,45,30,0.42), 0 0 0 1px rgba(58,51,43,0.06)",
        };

    const stageLabel = camera
      ? "Set up your shoot"
      : capturing
        ? "Capturing"
        : "Review & export";
    const statusDot = s.streamReady
      ? "#5C9A6B"
      : s.cameraError
        ? "#C16A4A"
        : "#C9B79C";
    const statusText = s.streamReady
      ? "Camera live"
      : s.cameraError
        ? "No camera"
        : "Connecting";
    const progressText =
      "shot " + Math.min(s.captureIndex + 1, slots) + " of " + slots;
    const camIcon = (extra) => (
      <svg
        width="19"
        height="19"
    ...
    return (
      <div
        ...
        style={{
          ...
          color: "#3A332B",
          background:
            "radial-gradient(125% 95% at 50% 0%, #F8F3E9 0%, #EFE7D7 58%, #E6DBC6 100%)",
        }}
      >
        <div style={{ width: "100%", maxWidth: "1040px", margin: "0 auto" }}>
          <div className="pb-cabinet" style={cabinetStyle}>
            {curtains && (
              <div style={{ position: "relative", marginBottom: "2px" }}>
                <div
                  style={{
                    ...
                    boxShadow:
                      "inset 0 1px 0 rgba(255,255,255,0.2), inset 0 -7px 13px rgba(0,0,0,0.18)",
                  }}
                >
                  PHOTO&nbsp;&nbsp;BOOTH
                </div>
              ...
              {curtains && (
                <div
                  style={{
                    ...
                    boxShadow:
                      "inset -6px 0 12px rgba(0,0,0,0.28), inset 6px 0 8px rgba(0,0,0,0.18)",
                  }}
                />
              )}
                  ...
                  >
                    ...
                    {camera && !!s.cameraError && !s.streamReady && (
                      ...
                      >
                        ...
                        >
                          {errMap[s.cameraError] ||
                            "Something went wrong reaching the camera."}
                        </div>
                        <button
                          type="button"
                          ...
                          >
                            {FILTERS.map((f) => (
                              <button
                                ...
                                type="button"
                                onClick={() =>
                                  this.setState({ filterId: f.id })
                                }
                                style={
                                  s.filterId === f.id
                                    ? merge(pillBase, pillAct)
                                    : pillBase
                                }
                              >
                                {f.label}
                        ...
                        <div style={{ display: "flex", gap: "8px" }}>
                          <button
                            ...
                            onClick={() =>
                              this.setState((st) => ({
                                facing:
                                  st.facing === "user" ? "environment" : "user",
                              }))
                            }
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              this.setState((st) => ({ soundOn: !st.soundOn }))
                            }
                            style={soundStyle}
                          >
                          ...
                          {Array.from({ length: slots }).map((_, i) => (
                            <span
                              ...
                              style={{
                                ...
                                borderRadius: "50%",
                                background:
                                  i < s.heroCount ? ACC : "rgba(58,51,43,0.16)",
                    ...
                    {review && (
                      ...
                      >
                            ...
                            {FRAMES.map((f) => {
                              ...
                              return (
                                <button
                                  ...
                                  title={f.name}
                                  onClick={() =>
                                    this.setState({ frameColor: f.hex })
                                  }
                                  style={merge(swBase, {
                                    background: f.hex,
                                    boxShadow: act
                          ...
                          >
                            {BORDERS.map((b) => (
                              <button
                                ...
                                type="button"
                                onClick={() =>
                                  this.setState({ borderStyle: b.id })
                                }
                                style={
                                  s.borderStyle === b.id
                                    ? merge(pillBase, pillAct)
                        ...
                        <button
                          ...
                          onClick={() => {
                            if (s.stripUrl)
                              this.download(s.stripUrl, "photostrip.png");
                          }}
                          style={startBtnStyle}
                        >
                          Download photo (PNG)
                          ...
                          >
                            ...
                            >
                              ...
                              <button
                                ...
                                onClick={() => {
                                  if (s.gifUrl)
                                    this.download(s.gifUrl, "flipbook.gif");
                                }}
                                style={secondaryStyle}
                              >
                                Download GIF
